        ...
        for (Leds *leds: listOfLeds) {
          ...
          if (leds->doMap) { //add pixel in leds mappingtable
            ...
            if (pixel >= startPosAdjusted && pixel <= endPosAdjusted ) { //if pixel between start and end pos
              ...
              Coord3D mapped;
              // Adjust Size and Pixel
              (projection->*leds->adjustSizeAndPixelCached)(*leds, sizeAdjusted, pixelAdjusted, midPosAdjusted, true);  
              // Adjust leds->size if not done yet
              ...
              // Adjust Mapped
              (projection->*leds->adjustMappedCached)(*leds, mapped, sizeAdjusted, pixelAdjusted, midPosAdjusted, true);
